...
class WebsiteControllerTest {
        ...
        void should_returnPaginatedWebsites_when_getWebsites() throws Exception {
                ...
                when(websiteService.getWebsites(anyString(), isNull(), isNull(), isNull(), isNull(), anyInt(), anyInt(), anyString(),
                                anyString()))
                                .thenReturn(paginatedData);

                // When & Then
                mockMvc.perform(get("/open/websites").param("channel", "default"))
        ...
        void should_returnBadRequest_when_invalidLanguage() throws Exception {
                // When & Then
                mockMvc.perform(get("/open/websites?lang=INVALID_LANG").param("channel", "default"))
                                .andExpect(status().isOk())
